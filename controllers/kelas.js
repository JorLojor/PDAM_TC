      ...
      const data = await KelasModel.find()
        ...
        for (const kelas of data) {
            for (let i = 0; i < kelas.peserta.length; i++) {
              ...
              const userData = await UserModel.findOne({ _id: peserta.user });
              if (userData) {
            ...
            for (let i = 0; i < kelas.instruktur.length; i++) {
                ...
                const userData = await UserModel.findOne({ _id: instruktur.user });
                if (userData) {
    ...
    try {
      ...
      if (!resultkelas.peserta.includes(idUser)) {
        if (
          ...
          const resultUserSave = await resultUser.save({ session });
