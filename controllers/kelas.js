...
module.exports = {
     ...
     nilaiPerKelas: async (req, res) => {
          ...
          try {
               const kelas = await KelasModel.findById(kelasId).populate(
                    "materi"
               );
               if (!kelas) {
               ...
               const totalNilai = nilaiPermateri.reduce(
                    (acc, curr) => acc + curr,
                    0
               );
               ...
               response(
                    200,
                    kelas,
                    "Nilai rata-rata kelas berhasil di update",
                    res
               );
          } catch (error) {
     ...
     enrolKelas: async (req, res) => {
          ...
          try {
               ...
               if (!resultkelas.peserta.includes(idUser)) {
                    if (
                         (resultkelas.kelasType === 1 &&
                              resultUser.userType === 1) ||
                         (resultkelas.kelasType === 0 &&
                              (resultUser.userType === 1 ||
                                   resultUser.userType === 0))
                    ) {
