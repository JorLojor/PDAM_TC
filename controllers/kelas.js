...
module.exports = {
  ...
  getAbsensi: async (req, res) => {
    try {
      ...
      const targetClass = await KelasModel.findOne({
        _id: kelas,
        status: { $ne: "deleted" },
      });
      ...
      if (ids.length > 0) {
        ...
        if (kelas.length > 0) {
          for (let i = 0; i < kelas.length; i++) {
            ...
            if (
              new Date(kelas[i].jadwal[kelas[i].jadwal.length - 1].tanggal) >
              today
            ) {
              data.push({
                id: kelas[i]._id,
  ...
  deleteKelas: async (req, res) => {
    ...
    try {
      ...
      if (checkKelas.peserta.length !== 0) {
        ...
        await session.commitTransaction();
        response(200, checkKelas, "berhasil", res);
        return false;
      }
  ...
  enrolKelas: async (req, res) => {
    ...
    try {
      ...
      resultUser.kelas.map(async (k) => {
        console.log(
          k.kelas.toString(),
          resultkelas._id.toString(),
          k.kelas.toString() == resultkelas._id.toString()
        );
        if (k.kelas.toString() == resultkelas._id.toString()) {
          response(401, resultkelas, "Kelas sudah diikuti", res);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const isActive =
        req.query.isActive == null || req.query.isActive == undefined
          ? true
          : req.query.isActive;
      ...
      if (!req.query.page || req.query.page === 0) {
        ...
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
        let rawData = await KelasModel.find({
          ...req.body,
          isActive: isActive,
          status: { $ne: "deleted" },
        });
